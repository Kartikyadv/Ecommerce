...
export const logoutUser = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json(responseResolver(200, true, "Logged Out"));
});
...
export const getUserDetails = catchAsyncError(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(responseResolver(200, true, `got user detail`, user));
});
...
export const getAllUser = catchAsyncError(async (req, res, next) => {
  ...
  res.send(responseResolver(200, true, "Got All User", users));
});

// Get other User detail - ADMIN
export const getOtherUserDetail = catchAsyncError(async (req, res, next) => {
  ...
  res.send(responseResolver(200, true, "Got User", user));
});
...
export const updateUserRole = catchAsyncError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(`User does not exist with userId: ${req.params.id}`)
    );
  }
...
export const deleteUser = catchAsyncError(async (req, res, next) => {
  ...
  res.send(responseResolver(200, true, "User Deleted"));
});
